...
class Acf_Mapping {
  ...
  const ASSOCIATIONS = [
    'studio' => [
      ...
      'field_5cfc0d0b06a38' => 'social_media_youtube_profile'
    ],
    'instructor' => [
      ...
      'field_5cfc0ca8e792d' => 'social_media_youtube_profile',
    ]
  ];

  // the following fields are ignored entirely. they are taxonomy terms.
  ...
  public function __construct ($post_type) {
    ...
    $this->associations = self::ASSOCIATIONS[$this->post_type];
  ...
  public function ignore_updates ( $value, $post_id, $field ) {
    ...
    if ( array_key_exists($field['key'], $this->associations) || in_array($field['key'], self::IGNORED)) {
      return null;
    }
  ...
  function save_post( $post_id ) {
    ...
    if( is_admin() && (!empty($_POST['acf']) || get_post_type($post_id) === $this->post_type) ){
      $id = $post_id;
      ...
      if($this->row_exists($post_id)){
        ...
        $this->entry_table->insert($results);
      }
    }
  }
  ...
  public function map_group ($fields, &$data = [], $post_id) {
    foreach($fields as $key => $value) {
      ...
      // remove escape characters from the 'about me' field
      $value = ($key === 'field_5cfaf4eb0c633') ? stripslashes($value) : $value;
  ...
  function load_field( $value, $post_id, $field ) {
    ...
    // field_5cfaf72026b12
    // echo $field['key']. ' ' . $field['name'].'<br>';
    if($field['name'] === 'profile_image') {
      $field['name'] = 'basic_info_profile_image';
    }
